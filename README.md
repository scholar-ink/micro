...
docker run -d  --link=etcd   --name micro -p 8080:8080 -e MICRO_API_NAMESPACE=test.retail.api  -e MICRO_REGISTRY=etcd -e MICRO_REGISTRY_ADDRESS=http://etcd:2379 --restart=always  registry.cn-shanghai.aliyuncs.com/youdian/micro api --handler=rpc
docker run -d  --link=etcd   --name micro_api -p 8081:8080 -e MICRO_API_NAMESPACE=test.retail.api  -e MICRO_REGISTRY=etcd -e MICRO_REGISTRY_ADDRESS=http://etcd:2379 --restart=always  registry.cn-shanghai.aliyuncs.com/youdian/micro api --handler=api
MICRO_API_NAMESPACE=test.retail.api MICRO_REGISTRY=etcd ./micro api  --handler=rpc
